...
Formally:
- Let a 3D point \( \mathbf{X}(t) \) project to image coordinates \( (x(t), y(t)) \).
...
- translational velocity \(\mathbf{T} = (T_x, T_y, T_z)\),
- rotational velocity \(\boldsymbol{\omega} = (\omega_x, \omega_y, \omega_z)\),
