...
# 2) Quadratic: y = a2 x^2 + a1 x + a0
X2 = np.column_stack([x**2, x, np.ones_like(x)])
a2, a1, a0 = np.linalg.lstsq(X2, y, rcond=None)[0]

# 3) Cubic: y = c3 x^3 + c2 x^2 + c1 x + c0
X3 = np.column_stack([x**3, x**2, x, np.ones_like(x)])
c3, c2, c1, c0 = np.linalg.lstsq(X3, y, rcond=None)[0]

# 4) "Bicubic" in 1D is not a distinct model.
# Bicubic interpolation is a 2D method (x,y -> z) on a grid.
# Here we interpret your request as a higher-order polynomial in 1D.
# We'll use degree-5 as a reasonable "more flexible than cubic" model:
# y = d5 x^5 + d4 x^4 + d3 x^3 + d2 x^2 + d1 x + d0
...
def draw_panel(ax_i, title, y_fit_line, eq_text):
    ...
    ax_i.plot(x_line, y_fit_line, linewidth=2.6, label="fit")
