...
# ---- Modern plot: RMSE line + markers (inspired by the reference style) ----
plt.rcParams.update({
  "figure.dpi": 170,
  "font.size": 11
})
...
# line + round markers (use default matplotlib color cycle)
ax.plot(
    xpos, rmses,
    linewidth=2.2,
    marker="o",
...
# Light grid (both directions) + softened axes
...
# Tight y-range + headroom for labels
...
for x_i, v in zip(xpos, rmses):
  ax.text(x_i, v + 0.22 * pad, f"{v:.2f}", ha="center", va="bottom", fontsize=10, color="#444444")

# Legend along the bottom (like the reference)
